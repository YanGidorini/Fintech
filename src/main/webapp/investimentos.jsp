            		...
            		<c:when test="${not empty investimentos}">
            			<section class="investimento">
            				<c:forEach items="investimentos" var="investimento">
								<div class="row mt-3">		            	   	
									<div class="col-md">
										<p class="font-color-1 text-end mb-2">${investimento.dtRealizacao}</p>
									
										<div class="card-default p-3 ps-4 text-start position-relative">
											<div class="categoria_tag-inline categoria_tag--lci position-absolute"></div>
												<p class="mb-1 investimentos__corretora">${investimento.corretora.nmCorretora}</p>
												<p class="pb-2 mb-2 border-bottom border-2">${investimento.nome}</p>
											<div class="d-flex justify-content-between align-items-end">
												<span class="investimentos__valor"><fmt:formatNumber value="${investimento.valor}" type="currency"/></span>
												<span class="investimentos__corretora">vence ${investimento.dtVencimento}</span>
											</div>
										</div>                                      
									</div>  
									            
									<div class="col-auto d-flex mt-3 mt-sm-0">
										<button class="button me-2 me-sm-3" type="button" title="Editar despesa" data-bs-toggle="modal" data-bs-target="#editModal" onclick="edition.value = '${investimento.idInvestimento}'; nome.value = '${investimento.nome}'; valor.value = '${investimento.valor}'; data.value = '${despesa.dtDespesa}' ; time.value = '${despesa.hora}'; categoriaDespesa.value = '${despesa.categoria.nmCategoria}'; selectCategoria() ">
											<svg viewBox="0 0 24 24">
											    <path  d="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z" />
											</svg>
										</button>
										                  
										<button class="button" type="button" title="Excluir despesa" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="exclusion.value = ${investimento.idInvestimento}">
											<svg viewBox="0 0 24 24">
